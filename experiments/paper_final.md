...
- Decoder: \(\text{dec}(z) = W_{\text{dec}} z + b_{\text{dec}}\) where \(W_{\text{dec}} \in \mathbb{R}^{d_h \times d}\) has unit-norm **rows** (each row is a 1536-dim decoder direction for one feature, normalized to unit norm; equivalently, the code stores W_dec as (4096, 1536) with rows normalized), \(b_{\text{dec}} \in \mathbb{R}^d\).
- Full SAE: \(\text{SAE}(x) = W_{\text{dec}} \cdot \text{TopK}(W_{\text{enc}}^T x + b_{\text{enc}}, k) + b_{\text{dec}}\).

The decoder column space projector is \(P_{\mathcal{F}} := W_{\text{dec}} W_{\text{dec}}^+\) where \(W_{\text{dec}}^+\) is the Moore-Penrose pseudoinverse. With the SAE overcomplete (\(d_h = 4096 \gg d = 1536\)), the column space \(\operatorname{col}(W_{\text{dec}}) \subset \mathbb{R}^{d_h}\) is a 1536-dimensional subspace in the 4096-dim feature space; its **image in activation space** via the linear map \(W_{\text{dec}} : \mathbb{R}^{d_h} \to \mathbb{R}^d\) spans essentially all of \(\mathbb{R}^d\) (the 4096 unit-norm rows are sufficient to span the 1536-dim activation space), so \(P_{\mathcal{F}} \approx I_d\) in practice. The linearized projection is \(P_{\mathcal{F}}^{\text{lin}} := W_{\text{dec}} W_{\text{enc}}^T \in \mathbb{R}^{d \times d}\). Note that \(P_{\mathcal{F}} x \in \operatorname{col}(W_{\text{dec}})\) for all \(x\), and \((I - P_{\mathcal{F}}) W_{\text{dec}} = 0\).
...
| A4b | Benign gradient harm-small: \(\|\langle \nabla g(h^0_p), \hat{v}_{\text{harm}} \rangle\| \leq \epsilon\) for all \(p \in \mathcal{B}\), with \(\epsilon \ll 1\) (empirically, \(\cos(v_{\text{drug}}, v_{\text{harm}}) = -0.10\) implies \(\epsilon \lesssim 0.1\) for the dominant confident-tone readout) | T4.3 |
...
- \(\gamma_M \leq 0.065\) z-units per unit \(\alpha\) for the harm direction (from E2 z-growth rate, after Code↔Math audit)
...
3. **(Benign-Prompt Invariance.)** Assume **(A4b-approx)**: On benign prompts, the harm-direction component of the readout gradient is bounded: \(\|\langle \nabla g(h^0_p), \hat{v}_{\text{harm}} \rangle\| \leq \epsilon\) for all \(p \in \mathcal{B}\), with \(\epsilon \ll 1\). (Empirically, \(\cos(v_{\text{drug}}, v_{\text{harm}}) = -0.10\) suggests the dominant confident-tone readout has \(\epsilon \lesssim 0.1\).) Let \(h^0\) be a clean activation and \(g: \mathbb{R}^d \to \mathbb{R}\) a smooth readout. If additionally \(\langle h^0, v_{\text{harm}} \rangle = 0\), then:
...
**The projector-based bound is vacuous for overcomplete SAEs.** Direct computation via `audit_sae_projection.py` shows \(P_{\mathcal{F}} = W_{\text{dec}}^+ W_{\text{dec}}\) has eigenvalues in [0.999993, 1.000008] — essentially the identity. Consequently \(\eta_v = \|(I - P_{\mathcal{F}}) v_{\text{harm}}\| \approx 3 \times 10^{-6}\) (machine precision) for all four tested directions. The overcomplete SAE (d_h=4096 unit-norm rows in 1536-dim activation space) spans all of \(\mathbb{R}^d\). The T2 inequality is mathematically valid but the bound is vacuous: the predicted overdose threshold \(\alpha_{\text{OD}} \to \infty\) from the projector component alone.

**The empirical z-growth is dominated by the TopK approximation error.** Although \(\eta_v \approx 0\), the SAE still fails to perfectly reconstruct the activation because only \(k=32\) of 4096 features are active per token. This TopK-induced approximation error is the actual source of the observed z-growth. The empirical rates (from re-run of `e2_controls.py` with 9 α values × 4 directions × 5 prompts, saved to `controls_v2.json`):

| Direction | Mean z-growth rate | Per-prompt rates |
|-----------|-------------------:|-------------------|
| harm      | **0.0650 z/α**    | +0.046, +0.068, +0.056, +0.094, +0.062 |
| drug      | 0.0228 z/α        | +0.019, +0.026, +0.017, +0.027, +0.025 |
| rand      | 0.0380 z/α        | +0.029, +0.033, +0.037, +0.052, +0.039 |
| perp      | 0.0384 z/α        | +0.029, +0.033, +0.037, +0.053, +0.039 |

- **Directional specificity (the rigorous T2 claim):** harm/drug = **2.85×**, harm/rand = 1.71×, harm/perp = 1.69×. The harm direction causes 2.85× more z-growth than the drug direction at the same unit-norm α. The random and perpendicular-to-both directions give essentially identical rates (0.0380 vs 0.0384), confirming that the directional signal is specific to harm, not a generic steering artifact. **This rules out FM1** (trivial correlation).

- **Monotonicity:** Per-prompt z-growth is positive for all 5 harm-direction prompts across α ∈ {0, +12}, validating the linear growth model for the empirical z(α) curve (though the linear lower bound from the projector residual is vacuous).

- **Overdose in practice:** At raw c ≈ 1.5, the empirical generation quality degrades (random character runs, philosophical drift). This overdose onset is NOT explained by the projector-based \(\alpha_{\text{OD}}\) — it occurs at much smaller α because the TopK approximation error scales with α and the model's effective tolerance is reached earlier. The empirical overdose onset is therefore the practical bound, not the projector-derived one.

**Conclusion for T2:** The qualitative claim (directional specificity, monotonicity, harm > drug > non-harm) is robust and verified. The quantitative claim (\(\alpha_{\text{OD}}\) as a computable safety threshold) is **revised**: in the overcomplete-SAE regime, the projector component is near-zero, and the actual overdose boundary must be characterized empirically rather than analytically. A future refinement would use the per-prompt TopK approximation error as the rate constant in a corrected T2.
...
| **T1** | Geometry sensitivity (harm) | \(\gamma_M\) | Bounded (A4) | **0.065 z/α** (harm direction, unit-norm) | E2 controls_v2: 5 prompts | Satisfies A4 (revised from 0.80 after Code↔Math audit) |
| **T1** | Geometry sensitivity (drug) | \(\gamma_M\) | Bounded (A4) | **0.023 z/α** (drug direction, unit-norm) | E2 controls_v2: 5 prompts | Satisfies A4 (revised from 0.22) |
...
| **T2** | Off-manifold sensitivity (harm) | \(\eta_v\) | \(\|(I-P_{\mathcal{F}})v_{\text{harm}}\| > 0\) | \(\eta_v^{\text{(direct)}} \approx 3 \times 10^{-6}\) (machine-ε) | `audit_sae_projection.py`: P_F = W_dec^+ @ W_dec computed directly | **P_F ≈ I** (eigvals 0.9999–1.0000): overcomplete SAE (d_h=4096 ≫ d=1536) spans all of R^d, so projector-based η_v cannot explain empirical z-growth. TopK approximation error dominates. |
| **T2** | z-growth rate (harm) | empirical (NOT \(\eta_v/\sigma_{\text{rec}}\)) | \(> 0\) | **0.0650 z/α** (5 prompts) | E2 controls_v2: linear regression over α ∈ {0, +12} | Empirically observed, NOT predicted by η_v/σ_rec |
| **T2** | z-growth rate (drug, control) | empirical | \(< \gamma_M^{\text{harm}}\) (directional specificity) | **0.0228 z/α** (5 prompts) | E2 controls_v2 | Qualitatively smaller |
| **T2** | Harm vs. non-harm specificity | ratio of z-growth | \(> 1\) | **2.85×** (harm/drug), 1.71× (harm/rand), 1.69× (harm/perp) | E2 controls_v2: 4-direction comparison | **Rules out FM1** (trivial correlation) |
| **T2** | Overdose threshold (projector-based) | \(\alpha_{\text{OD}} \leq (\tau_M\sigma_{\text{rec}}+\mu_{\text{rec}}+\text{const})/\eta_v\) | bound from P_F | Bound is vacuous (η_v ≈ 0) → no useful T2 prediction from projector | E2: bound vacuous; empirical overdose at c ≈ 1.5 (raw) | **Projector-based bound is not useful for overcomplete SAEs** |
| **C5.1** | Titrated recovery | empirical | \(> 0\) (predicted) | 0.00 → 0.33 (back to baseline) | E4: titrated vs. static antidote | **Confirmed**: P-controller beats open-loop |
| **C5.1** | Stability condition | \(\lambda < 2/\eta_{\text{ant}}^2\) | \(0.5 < 2/\eta_{\text{ant}}^2\) | \(\eta_{\text{ant}} \approx 3 \times 10^{-6}\) → \(2/\eta_{\text{ant}}^2 \approx 2.2 \times 10^{11}\) | `audit_sae_projection.py` | **Holds by 4.5×10¹¹× margin**: λ=0.5 is trivially safe |
...
**Cross-model SAE baseline mismatch (VULN-CROSS-SAE-001).** The SAE used in the verification table (μ_rec = 17.15, σ_rec = 5.94) was trained on fp16 Qwen-2.5-1.5B activations at layer 12 (d=1536). Gemma-2-2B (d=2304) and Gemma-4-E4B (d=2560) have larger d, and their RMSNorm-scaled residual norms are larger by a factor of √(d_Gemma / d_Qwen) ≈ 1.22 and 1.29 respectively. The expected μ_rec for these models is therefore ≈ 17.15 × 1.22 ≈ 21.0 (Gemma-2-2B) and ≈ 17.15 × 1.29 ≈ 22.1 (Gemma-4-E4B). The T2 overdose bound (eq. 3.1) and the E2 z-growth measurements for Gemma models are therefore mis-calibrated by ~20-30% when using Qwen-trained SAE statistics. The directional-specificity claim (harm > drug) is preserved because the multiplier is roughly uniform across directions. The geometric T4 claims (norm preservation, harm orthogonality) are unaffected. **Mitigation:** a per-model SAE (or per-model μ_rec/σ_rec rescaling) is needed for cross-model z-score comparisons; this is left for future work.

**Empirical η_v vs predicted z/α.** The T2 proof gives \(c_1 = \eta_v/\sigma_{\text{rec}}\) as the lower bound on z-growth, where \(\eta_v = \|(I - P_{\mathcal{F}})v\|\) is the projector residual. In our setup, \(P_{\mathcal{F}} \approx I\) because the SAE is overcomplete (\(d_h = 4096 \gg d = 1536\)), so **direct computation gives \(\eta_v \approx 3 \times 10^{-6}\) for all four tested directions** (machine-ε, see `audit_sae_projection.py`). This is **orders of magnitude smaller** than the empirical z-growth of 0.065 z/α for the harm direction. The observed z-growth is dominated by the **TopK approximation error** (only k=32 of 4096 features are active per token), not by the projector residual. T2's inequality is therefore correct but extremely loose; the rigorous empirical claim is the **directional specificity** (harm/drug = 2.85×, harm/rand = 1.71×, harm/perp = 1.69×), which rules out FM1. The absolute z/α rate is set by the TopK approximation error, not by the projector residual, and is model- and architecture-specific.
